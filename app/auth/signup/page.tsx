...
export default function SignupPage() {
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (result?.error) {
        setError("Compte créé, mais erreur de connexion. Réessayez.");
        return;
      }
  ...
  return (
    <div className="fixed inset-0 z-[999] flex items-center justify-center overflow-y-auto bg-night py-8">
      ...
      <div className="relative z-10 w-full max-w-md rounded-3xl border border-neon-pink/40 bg-gradient-to-b from-obsidian to-night p-10 shadow-glow">
        ...
        <form onSubmit={handleSubmit} className="mt-8 space-y-5">
          <div>
            <label htmlFor="name" className="block text-sm font-semibold uppercase tracking-[0.2em] text-white/80">
              Nom (optionnel)
            </label>
            <input
              id="name"
          ...
          <div>
            <label htmlFor="confirmPassword" className="block text-sm font-semibold uppercase tracking-[0.2em] text-white/80">
        ...
        <p className="mt-8 text-center text-sm text-white/60">
          Déjà un compte ?{" "}
          <Link href="/auth/login" className="font-semibold text-neon-pink transition hover:text-neon-glow">
            Se connecter
          </Link>
        </p>
      </div>
